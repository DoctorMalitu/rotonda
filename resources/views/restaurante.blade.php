...
<form action="{{ route('restaurante.crear') }}" method="POST">
	...
	<div class="form-group">
	<input type="text" name="direccion" placeholder="Direccion" class="form-control mb-2 {{ $errors->has('direccion') ? 'is-invalid' : '' }}" value="{{ old('direccion') }}">
	{!! $errors->first('direccion','<span class="help-block invalid-feedback">:message</span>') !!}
</div>
<div class="form-group">
...
<table class="table mt-5">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><a href="{{ route('restaurante.editar',$item) }}" class="btn btn-warning btn-sm">Editar</a>

		<form action="{{ route('restaurante.eliminar',$item) }}" method="POST" class="d-inline">
			@method('DELETE')
			@csrf
