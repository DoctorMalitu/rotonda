...
@endif
<form action="{{ route('recetas.crear') }}" method="POST">
	@csrf
...
<table class="table mt-5">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><a href="{{ route('recetas.editar',$item) }}" class="btn btn-warning btn-sm">Editar</a>

		<form action="{{ route('recetas.eliminar',$item) }}" method="POST" class="d-inline">
			@method('DELETE')
			@csrf
			<button class="btn btn-danger btn-sm" type="submit">Eliminar</button>

		</form>
    <a href="{{ route('personalizar',$item) }}" class="btn btn-primary btn-sm">Personalizar</a>
      </td>
    </tr>
	@endforeach
  </tbody>
